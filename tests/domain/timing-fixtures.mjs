...
const DEFINITIONS=[
  ...
  ['T04-offplan-no-lag','Off-plan sale with no escrow lag','IMPLEMENTED',o=>{set(o,'strategy.offPlanSale.enabled',true);set(o,'strategy.offPlanSale.escrowLagYears',0);} ],
  ...
  ['T08-offplan-lag1','Off-plan collection extends one year beyond nominal tranche','IMPLEMENTED',o=>{set(o,'development.constructionYears',3);set(o,'strategy.offPlanSale.enabled',true);set(o,'strategy.offPlanSale.escrowLagYears',1);} ],
  ['T09-offplan-lag2-backloaded','Back-loaded off-plan with two-year escrow lag','IMPLEMENTED',o=>{set(o,'development.constructionYears',3);set(o,'strategy.offPlanSale.enabled',true);set(o,'strategy.offPlanSale.escrowLagYears',2);set(o,'strategy.offPlanSale.curve','back_loaded');}],
  ['T10-vat-refund-lag0','Recoverable VAT with zero refund lag','IMPLEMENTED',o=>{set(o,'vat.enabled',true);set(o,'vat.inputRecoveryPct',1);set(o,'vat.refundLagYears',0);} ],
  ['T11-vat-refund-lag2','Recoverable VAT with two-year refund lag','IMPLEMENTED',o=>{set(o,'development.constructionYears',3);set(o,'development.operationYears',2);set(o,'vat.enabled',true);set(o,'vat.inputRecoveryPct',1);set(o,'vat.refundLagYears',2);} ],
  ['T12-phased-subdivision','Four-year serviced-land absorption after construction','IMPLEMENTED',o=>{set(o,'development.scopeType','infra_only');set(o,'development.infraCostPerSqm',800);set(o,'development.constructionYears',2);set(o,'subdivision.phasedAbsorption',true);set(o,'subdivision.absorptionYears',4);set(o,'subdivision.curve','even');}],
